...
/// - The copy is repacked to NV21, the byte format accepted by on-device
///   ML Kit (`fromBytes` rejects BGRA on Play Services runtimes).
/// - Any failure (missing plugin on Web/desktop, missing Play Services
///   model, corrupt bytes) yields `[]`. The rule engine then simply finds
///   no text hits; the selected image stays editable with a manual-review
///   notice instead of being discarded.
...
class MlKitTextDetector implements app.TextRegionDetector {
  ...
  Future<List<app.RecognizedTextRegion>> detect(
    ...
    try {
      final prepared = await compute(_prepareNv21ForText, <String, Object>{
...
Map<String, Object>? _prepareNv21ForText(Map<String, Object> payload) {
